...
export function ConnectWallet() {
  ...
  const handleConnect = () => {
    ...
    if (isInFarcaster) {
      // In Farcaster - use the miniApp connector which will show wallet selection
      const farcasterConnector = connectors.find(
        (connector) => connector.id === "farcasterMiniApp"
      )
      
      if (farcasterConnector) {
        console.log("Connecting with Farcaster connector - user will choose wallet")
        connect({ connector: farcasterConnector })
        return
      }
    }

    // Outside Farcaster - find and prioritize standard connectors
    const standardConnectors = connectors.filter(
      (connector) => connector.id !== "farcasterMiniApp"
    )

    // Priority order: injected (MetaMask), metaMask, coinbaseWallet, walletConnect
    const connectorPriority = [
      (c: any) => c.id === "injected",
      (c: any) => c.id === "metaMask", 
      (c: any) => c.id === "coinbaseWallet",
      (c: any) => c.id === "walletConnect",
      () => true // any other connector
    ]

    let selectedConnector = null
    for (const priorityFn of connectorPriority) {
      selectedConnector = standardConnectors.find(priorityFn)
      if (selectedConnector) break
    }
